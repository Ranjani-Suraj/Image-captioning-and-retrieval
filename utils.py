...
class Ranker():
    ...
    def get_item(self, index):
        ...
        return image_path, data[id]

    def get_items(self, indexes):
        items = [self.get_item(i) for i in indexes]
        # items = Parallel(n_jobs=self.num_workers)(
            # delayed(self.get_item)(
                # i) for i in indexes)
        image_path, meta_info = zip(*items)
        return image_path, meta_info

    def update_emb(self, model, batch_size=64):
        data_emb = []
        data_asin = []
        ...
        for i in range(num_batch):
            ...
            image_path, asins = self.get_items(batch_ids)
            with torch.no_grad():
                feat = model.encode_image(image_path).float()
